...
;(async () => {
  let baseUrl = 'https://www.douban.com/group/haixiuzu/discussion?start=25'
  let topicDetail = await getIndexTopics(baseUrl, baseUrl)
  await sleep(1000)
  for (let item of topicDetail) {
    await sleep(1000)
    downloadPics(await getTopicDetail(item.href, baseUrl))
  }
})()
